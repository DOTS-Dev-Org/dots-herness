...
public sealed class LocalRuntime
{
    ...
    public string BinaryPath => Path.Combine(Paths.Runtime, "llama-server");
    public string ArchivePath => Path.Combine(Paths.Runtime, "llama.cpp.tar.gz");
    ...
    public async Task EnsureRuntimeAsync(Action<FileDownloader.Progress>? progress = null, CancellationToken ct = default)
    {
        ...
        var asset = await LatestLinuxAssetAsync(ct);
    ...
    public async Task<ReleaseAsset> LatestLinuxAssetAsync(CancellationToken ct = default)
    {
        ...
        return LinuxAsset(json, HostArch);
    }

    public static ReleaseAsset LinuxAsset(JsonValue json, string arch)
    {
        ...
        throw new RouterException($"No Linux {arch} llama.cpp build in {tag}");
    }
    ...
    private void UnpackRuntime(string archive)
    {
        ...
        var tar = new Process
        {
            ...
            {
                FileName = "tar",
                ArgumentList = { "-xzf", archive, "-C", extract },
                UseShellExecute = false,
                RedirectStandardOutput = true,
                RedirectStandardError = true,
            },
        };
        if (!tar.Start()) throw new RouterException("failed to start tar");
        tar.WaitForExit();
        if (tar.ExitCode != 0) throw new RouterException($"tar failed ({tar.ExitCode})");
        var found = FirstFile("llama-server", extract)
